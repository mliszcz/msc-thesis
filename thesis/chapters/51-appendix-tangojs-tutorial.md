...
he/she wants to use the simple in-memory *Connector*. However, for accessing a
real TANGO infrastructure, a working instance of mTango is required.
...
development. It may be used with the existing TANGO deployment. The rest of
...
a desired backend and pull the dependencies. The steps are as follows:
...
in order to create an own web-based TANGO client application. Any libraries
...
function. The process described here is shown in [@Lst:A-tangojs-setup].
...
attribute. The code shown in [@Lst:A-tangojs-widgets] should be added to the
`body` tag.
