...
in IIFE and the `document` and `tangojs` global variables are injected.

```{#lst:B-html-blank .html}
<!DOCTYPE html>
<html>
  <template>
    <span id="label"></span>
  ...
  (function (document, tangojs) {
    'use strict'
  })(window.document, window.tangojs)
  </script>
</html>
```
...
**Populating the view.**
The widget will be a simple label that displays value of an attribute. A simple
`span` element will be enough. The template may be accessed from the Javascript
code by querying current document, like
`window.tangojs.web.util.getCurrentDocument().querySelector('template')`.
...
```{#lst:B-js-attributes .javascript}
